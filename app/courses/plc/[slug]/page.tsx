...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `https://www.learntechnique.com/courses/plc/${slug}` },
    openGraph: {
      title: `${title} | Technique Learning Solutions`,
      description: metaDescription,
      url: `https://www.learntechnique.com/courses/plc/${slug}`,
      type: "website",
    },
  };
}
...
export async function generateStaticParams() {
  ...
  if (!slugs.some((s: { slug: string }) => s.slug === AM2_COURSE_SLUG)) {
    slugs.push({ slug: AM2_COURSE_SLUG, category: "plc" });
  }
  return slugs;
}
...
async function CourseDetail({ params }: CoursePageProps) {
  ...
  const courseCanonicalUrl = `https://www.learntechnique.com/courses/plc/${slug}`;
  ...
  return (
    <main>
      ...
      <AnimatedSection variant="fade-up">
        <BookCourse data={bookData} courseUrl={`/courses/plc/${slug}`} />
      </AnimatedSection>
