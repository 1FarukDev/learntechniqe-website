...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `https://www.learntechnique.com/courses/aircon-refrigeration/${slug}` },
    openGraph: {
      title: `${title} | Technique Learning Solutions`,
      description: metaDescription,
      url: `https://www.learntechnique.com/courses/aircon-refrigeration/${slug}`,
      type: "website",
    },
  };
}
...
export async function generateStaticParams() {
  ...
  if (!slugs.some((s: { slug: string }) => s.slug === AM2_COURSE_SLUG)) {
    slugs.push({ slug: AM2_COURSE_SLUG, category: "aircon-refrigeration" });
  }
  return slugs;
}
...
async function CourseDetail({ params }: CoursePageProps) {
  ...
  const courseCanonicalUrl = `https://www.learntechnique.com/courses/aircon-refrigeration/${slug}`;
  ...
  return (
    <main>
      ...
      <AnimatedSection variant="fade-up">
        <BookCourse data={bookData} courseUrl={`/courses/aircon-refrigeration/${slug}`} />
      </AnimatedSection>
            ...
            lockedCourse={{
              title:
                typeof rawCourse?.title === "string"
                  ? rawCourse.title
                  : "Course",
              url: `/courses/aircon-refrigeration/${slug}`,
            }}
          />
        </AnimatedSection>
      )}
